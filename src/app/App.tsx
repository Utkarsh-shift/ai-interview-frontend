...
function App({ batch_id, job_id }: AppProps) {
  ...
  const screenRecordingBuffer = useRef<Blob[]>([]);
  const isScreenRecordingRef = useRef(false);
  const screenCaptureStream = useRef<MediaStream | null>(null);
  const isScreenCaptureStreamSetup = useRef(false);

  const mediaRecorderRef = useRef<MediaRecorder | null>(null);
  const mediaStreamRef = useRef<MediaStream | null>(null);
  const isVideoRecordingRef = useRef(false);
  const videoRecordingBuffer = useRef<Blob[]>([]);

  const [isRequestMade, setIsRequestMade] = useState(false); // Track if the request was made successfully
  ...
  interface StartScreenRecordingParams {
    sessionId: string;
    recordingId: string;
    onChunkReady: (chunk: Blob, type: 'scr_rec', partType: 'Part' | 'Final') => Promise<void>;
  }
  ...
  const permissionViolationCountRef = useRef(0);
  ...
  useEffect(() => {
    getAgents().then((agentList) => {
      const defaultName = agentList?.[0]?.name ?? "";
      setSelectedAgentConfigSet(agentList);
      setSelectedAgentName(defaultName);
    });
  }, []);
...
// }, [sessionStatus, permissions.screen, permissions.camera, sharedScreenStream, sessionId]);
  ...
  const requestScreenStream = async () => {
    try {
      ...
      setSharedScreenStream(screenStream); // This triggers the useEffect
...
// to check if internet is desconnected
  ...
  useEffect(() => {
    const handleFullscreenChange = async () => {
      ...
      if (interviewStarted && !document.fullscreenElement) {
        ...
        try {
          await captureWindow("Fullscreen exited", true); 
        } catch (err) {
    ...
    const handleVisibilityChange = () => {
      ...
      if (isHidden) {
        ...
        captureWindow("Window Lost Focus (Tab Switch)", false);
      }
    };
  ...
  const captureWindow = async (message: string, isFullscreenExit = false): Promise<void> => {
...
const updateSession = async (shouldTriggerResponse: boolean = false) => {
  ...
  const token =
    ...
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjo0OTAwMTEzOTM0LCJpYXQiOjE3NDY1MTM5MzQsImp0aSI6IjYwY2JiM2Y3ODdmZjQ3ZTJiZmVlNjMwNzQyNTNjZmYxIiwidXNlcl9pZCI6Mn0.STMAcG0Z3o4fpTXucNfnsPO9iQgmyqkNGqUs8gRE9HU";
